...
func TestHandler_Login(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Login. Internal Server Error.",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m.EXPECT().Login(c.Request().Context(), gomock.Any()).Return(rec.Header().Get("Authorization"), tt.error).AnyTimes()

			errAdd := handler.Login(c)
